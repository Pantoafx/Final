...
import re
import pickle
import sqlite3  # Import sqlite3 untuk koneksi database
...
def load_data_from_sql():
    ...
    conn = sqlite3.connect('data_alaska.db')  # Ubah ke path database Anda
    query = 'SELECT * FROM ulasan_alaska'  # Ubah nama tabel jika perlu
...
def main():
    ...
    tipe_barang_options = df['tipe_barang'].unique().tolist()
    ...
    if st.button('Analisis'):
        if tipe_barang_input:
            ...
            filtered_df = df[df['tipe_barang'] == tipe_barang_input]
    ...
    if st.button('Analisis Keseluruhan'):
        ...
        df['Prediksi Sentimen'] = df['ulasan'].apply(
            lambda x: predict_sentiment(x, modelsvc_loaded) if x is not None else 'Tidak ada ulasan'
        )
